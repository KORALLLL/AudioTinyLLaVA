...
class PretrainTemplate(Template):
    ...
    format_user: "Formatter" = EmptyFormatter(slot="<audio>") #EmptyFormatter(slot="<image>") 
    ...
    def make_labels(self, input_ids, prompt, tokenizer):
        ...
        mask_len = len(self.tokenizer_image_token("<audio>", tokenizer)) #len(self.tokenizer_image_token("<image>", tokenizer)) #
