...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'audio' in sources:
            ...
            # print(audio_file)
            flac, sample_rate = torchaudio.load(audio_file)
            flac = torchaudio.functional.resample(flac, 44100, 16000)
            # print(flac.squeeze().numpy().shape)
            data_dict['audio'] = flac.squeeze().numpy()
...
def pad_audios(audios, target_len):
    padded_audios = []
    for audio in audios:
        # Calculate the number of zeros needed to pad
        padding_needed = target_len - len(audio)
        if padding_needed > 0:
            # Pad the audio array with zeros at the end
            padded_audio = np.pad(audio, (0, padding_needed), mode='constant')
        else:
            # If the audio array is already larger than target_length, truncate it
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    #return dict(train_dataset=train_dataset if not is_eval else None,
    #            eval_dataset=train_dataset if is_eval else None,
